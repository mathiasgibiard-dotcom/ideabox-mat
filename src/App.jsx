...
export default function App() {
  ...
  function insertIdea(idea) {
    ...
    var row = {
      ...
      fonctionnalites: idea.fonctionnalites,
      folder: idea.folder,
      folder_color: idea.folderColor,
      status: idea.status,
  ...
  function startRecording() {
    ...
    r.onresult = function(e) { setRawInput(function(p) { return p ? p + " " + e.results[0][0].transcript : e.results[0][0].transcript; }); };
  ...
  function generateIdea() {
    ...
    fetch("/api/claude", {
      ...
      var parsed = JSON.parse(data.content && data.content[0] ? data.content[0].text : "{}");
      var idea = {
        id: Date.now(),
        raw: rawInput,
      ...
      <div style={{ background: "rgba(3,10,5,0.97)", borderBottom: "1px solid #00ff8825", padding: "14px 16px", position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(12px)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            <button onClick={function() { setScreen("ideas"); }} style={{ background: screen === "ideas" ? "rgba(0,255,136,0.08)" : "transparent", border: "1px solid " + (screen === "ideas" ? "#00ff8844" : "#00ff8818"), borderRadius: "4px", color: screen === "ideas" ? "#00ff88" : "#446655", padding: "7px 10px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.08em" }}>LISTE</button>
            <button onClick={loadIdeas} style={{ background: "transparent", border: "1px solid #00ff8818", borderRadius: "4px", color: "#00ff8844", padding: "7px 10px", cursor: "pointer", fontSize: "13px" }}>⟳</button>
          </div>
        </div>
      </div>
    );
        ...
        <div style={{ padding: "20px 16px", maxWidth: "700px", margin: "0 auto", position: "relative", zIndex: 1 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "10px", background: f.color + "12", border: "1px solid " + f.color + "44", borderRadius: "6px", padding: "10px 16px", marginBottom: "20px" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "10px", color: f.color + "77", marginTop: "2px" }}>{f.desc}</div>
            </div>
          ...
          <div style={{ background: "rgba(0,18,10,0.85)", border: "1px solid " + f.color + "22", borderRadius: "6px", padding: "18px", position: "relative" }}>
            <div style={{ fontSize: "10px", color: f.color + "77", letterSpacing: "0.18em", marginBottom: "12px" }}>DECRIS TON IDEE LIBREMENT</div>
  ...
  if (screen === "idea_detail" && activeIdea) {
    ...
    return (
      <div style={wrap}>
        <BG/><Header back={true} backLabel="LISTE" backAction={function() { setScreen("ideas"); }}/>
        <div style={{ padding: "20px 16px", maxWidth: "700px", margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ background: "rgba(2,14,8,0.9)", border: "1px solid #00ff8815", borderRadius: "6px", padding: "14px", marginBottom: "12px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {Object.keys(STATUS_CONFIG).map(function(k) {
                ...
                return <button key={k} onClick={function() { updateIdea(idea.id, { status: k }); }} style={{ flex: 1, background: idea.status === k ? v.color + "18" : "transparent", border: "1px solid " + (idea.status === k ? v.color + "77" : "#00ff8818"), borderRadius: "4px", color: idea.status === k ? v.color : "#446655", padding: "8px", cursor: "pointer", fontSize: "10px", letterSpacing: "0.08em", transition: "all 0.15s" }}>{v.label}</button>;
              })}
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={function() { setScreen("home"); }} style={{ flex: 1, background: "rgba(0,255,136,0.06)", border: "1px solid #00ff8833", borderRadius: "4px", color: "#00ff88", padding: "12px", cursor: "pointer", fontSize: "12px", fontWeight: "700", letterSpacing: "0.08em", textShadow: "0 0 10px #00ff88" }}>+ NOUVELLE IDEE</button>
    ...
    <div style={wrap}>
      ...
      <div style={{ padding: "16px", maxWidth: "700px", margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "16px" }}>
          <button onClick={function() { setFilterFolder("toutes"); }} style={{ background: filterFolder === "toutes" ? "rgba(0,255,136,0.08)" : "transparent", border: "1px solid " + (filterFolder === "toutes" ? "#00ff8844" : "#00ff8815"), borderRadius: "3px", color: filterFolder === "toutes" ? "#00ff88" : "#446655", padding: "5px 11px", cursor: "pointer", fontSize: "10px", letterSpacing: "0.08em" }}>TOUTES ({ideas.length})</button>
          {FOLDERS.filter(function(fl) { return fl.id !== "libre" && ideas.some(function(i) { return i.folder === fl.label; }); }).map(function(fl) {
            return <button key={fl.id} onClick={function() { setFilterFolder(fl.label); }} style={{ background: filterFolder === fl.label ? fl.color + "18" : "transparent", border: "1px solid " + (filterFolder === fl.label ? fl.color + "55" : "#00ff8815"), borderRadius: "3px", color: filterFolder === fl.label ? fl.color : "#446655", padding: "5px 11px", cursor: "pointer", fontSize: "10px", letterSpacing: "0.06em" }}>
          ...
          {filteredIdeas.map(function(idea, idx) {
            ...
            return (
              <div key={idea.id}
                ...
                onClick={function() { setActiveIdea(idea); setScreen("idea_detail"); }}
        ...
        {ideas.length > 0 && (
          <button onClick={function() { setScreen("home"); }} style={{ width: "100%", marginTop: "16px", background: "rgba(0,255,136,0.05)", border: "1px solid #00ff8822", borderRadius: "6px", color: "#00ff8866", padding: "14px", cursor: "pointer", fontSize: "11px", letterSpacing: "0.12em", fontFamily: "inherit" }}>+ NOUVELLE IDEE</button>
        )}
      </div>
      {toast && <Toast toast={toast}/>}<Styles/>
    </div>
